...
```r
install.packages(c("mice", "survey", "mitools"), repos = "https://cloud.r-project.org")
...
Cycle and survey-stratum indicators can be linearly dependent because the strata belong to particular cycles. MICE can therefore report removal of redundant predictors. This occurs in the generated survey test data; diagnostic events are printed for inspection and warnings are retained. Inspect `imp$loggedEvents` and convergence within the authorised analysis environment when running study inputs.
